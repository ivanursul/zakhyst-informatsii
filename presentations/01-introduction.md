...
| **Integrity** | Гарантія, що дані не змінені та не підроблені при передачі чи зберіганні |
...
Base64 — це **кодування**, не шифрування. Декодується миттєво:

```
bWFrc3ltOnN1cGVyc2VjcmV0  →  maksym:supersecret
```

**Проблеми:** credentials з кожним запитом, немає logout, немає делегування
...
**−** Stateful, cross-domain (cookie прив'язаний до домену)
**−** CSRF: браузер додає cookie автоматично — зловмисний сайт може надіслати запит від імені користувача

---

# Token-based Authentication

**Відмінність від сесій:** сервер не зберігає стан — вся інформація (ім'я, роль, термін дії) всередині токена. Сервер лише перевіряє підпис.
...
Client                    Server
  ...
  │←── { token: "eyJ.." }│
  │── GET /resource ─────→│ Верифікує підпис (без БД!)
  │←── 200 OK ───────────│
```

**+** Stateless, cross-domain, self-contained
**−** Revocation (вкрадений токен дійсний до закінчення `exp` — сервер не може його «забути»)
...
# Чим OAuth 2.0 відрізняється

Basic Auth, сесії, токени вирішують: **«хто я?»**
OAuth вирішує: **«як дати комусь обмежений доступ до моїх ресурсів без передачі пароля?»**
...
- Чіткий **поділ ролей**: хто дає доступ, хто просить, хто перевіряє
...
| | Basic Auth | Сесії | Токени | OAuth 2.0 |
|--|:---------:|:-----:|:------:|:---------:|
| Credentials кожен запит | так | — | — | — |
| Без стану на сервері | так | ні | так | так |
| Logout | ні | так | складно | так |
| Між доменами | ні | ні | так | так |
| Делегування доступу | ні | ні | ні | **так** |
| Гранулярний контроль | ні | ні | ні | **так** |

> «так» = підтримує, «ні» = не підтримує, «—» = не застосовується
...
**DFD (Data Flow Diagram)** — діаграма потоків даних: хто, куди і які дані передає

| DFD елемент | Що це | Загрози STRIDE |
|-------------|-------|----------------|
| External Entity | Користувач, зовнішній сервіс | S, R |
| Process | Логіка обробки даних | S, T, R, I, D, E |
| Data Store | База даних, файли | T, R, I, D |
| Data Flow | HTTP-запити, повідомлення | T, I, D |
